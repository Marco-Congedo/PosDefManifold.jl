...

{
    ...
    "title": "𝕄Vector type",
    "category": "section",
    "text": "𝕄Vector=Vector{𝕄}This is a vector of general Matrix matrices, alias of MatrixVector.    Julia sees is at: Array{Array{T,2} where T,1}.See aliases for the 𝕄 symbol and typecasting matrices for the use of matrices in PosDefManifold.𝕄Vector₂ type𝕄Vector₂=Vector{𝕄Vector} is a vector of 𝕄Vector type objects,   i.e., a vector of vectors of Matrices.   It is the alias of MAtrixVector₂.   Julia sees it as: Array{Array{Array{T,2} where T,1},1}.   Note that 𝕄Vector₂ is not a   matrix of matrices since the several 𝕄Vector objects it holds   do not need to have the same length."
},

{
    ...
    "title": "𝔻Vector type",
    "category": "section",
    "text": "𝔻Vector=Vector{𝔻}This is a vector of Diagonal matrices, alias of DiagonalVector.   Julia sees is at: Array{Diagonal,1}.See aliases for the 𝔻 symbol and typecasting matrices for the use of Diagonal matrices in PosDefManifold.𝔻Vector₂ type𝔻Vector₂=Vector{𝔻Vector} is a vector of 𝔻Vector type objects,  i.e., a vector of vectors of Diagonal matrices.  It is the alias of DiagonalVector₂.  Julia sees it as: Array{Array{Diagonal,1},1}. Note that 𝔻Vector₂ is not a  matrix of Diagonal matrices since the several 𝔻Vector objects it holds  do not need to have the same length."
},

{
    ...
    "title": "𝕃Vector type",
    "category": "section",
    "text": "𝕃Vector=Vector{𝕃}This is a vector of LowerTriangular matrices, alias of LowerTriangularVector.  Julia sees is at: Array{LowerTriangular,1}.See aliases for the 𝕃  symbol and typecasting matrices for the use of LowerTriangular matrices in PosDefManifold.𝕃Vector₂ type𝕃Vector₂=Vector{𝕃Vector} is a vector of 𝕃Vector type objects, i.e.,  a vector of vectors of LowerTriangular matrices.  It is the alias of LowerTriangularVector₂.  Julia sees it as: Array{Array{LowerTriangular,1},1}. Note that 𝔻Vector₂  is not a matrix of LowerTriangular matrices since the several 𝕃Vector  objects it holds do not need to have the same length."
},

{
    ...
    "title": "ℍVector type",
    "category": "section",
    "text": "ℍVector=Vector{ℍ}This is a vector of Hermitian matrices, alias of HermitianVector.  Julia sees is at: Array{Hermitian,1}.See aliases for the ℍ symbol and typecasting matrices for the use of Hermitian matrices in PosDefManifold.ℍVector₂ typeℍVector₂=Vector{ℍVector} is a vector of ℍVector type objects, i.e., a vector of vectors of Hermitian matrices.  It is the alias of HermitianVector₂.  Julia sees it as: Array{Array{Hermitian,1},1}. Note that ℍVector₂ is not a matrix of Hermitian matrices since the several ℍVector objects it holds do not need to have the same length."
},
...

{
    ...
    "title": "Threads",
    "category": "section",
    "text": "Some functions in PosDefManifold call BLAS routines for optimal performnce. This is reported in the help section of the concerned functions. In most functions julia calls BLAS routines automatically. You can set the number of threads the BLAS library should use by:using LinearAlgebra\nBLAS.set_num_threads(n)where n is the number of threads. By default, PosDefManifold reserves to BLAS all CPU threads available on your computer (given by the output of Sys.CPU_THREADS) minus the number threads used by Julia for multi-threaded computations (given by the output of Threads.nthreads()). In Linux and OSX this latter number of threads defaults to one and is controlled by an environment variable, i.e.,export JULIA_NUM_THREADS=4See this post, this post and julia doc on threads."
},
...

{
    ...
    "category": "function",
    "text": "(1) sumOfSqrDiag(X::𝕄)\n(2) sumOfSqrDiag(X::Union{𝔻, ℍ, 𝕃})\n\nalias: ssd\n\n(1) Sum of squares of the diagonal elements in real or complex Matrix X.  If X is rectangular, the main diagonal is considered.\n\n(2) Sum of squares of the main diagonal of real or complex Diagonal,  Hermitian or LowerTriangular matrix X.\n\nSee also: sumOfSqr, sumOfSqrTril.\n\nExamples\n\nusing LinearAlgebra, PosDefManifold\nX=randn(10, 20)\nsumDiag2=sumOfSqrDiag(X) # (1)\nsumDiag2=sumOfSqrDiag(𝔻(X)) # (2) 𝔻=LinearAlgebra.Diagonal\n\n\n\n\n\n"
},
...

{
    ...
    "category": "function",
    "text": "(1) pow(P::ℍ, args...)\n(2) pow(D::𝔻{T}, args...) where T<:Real\n\n(1) Given a positive semi-definite Hermitian matrix P, return the power  P^r_1 P^r_2  for any number of exponents r_1 r_2.  It returns a tuple of as many elements as arguments passed after P.\n\nP must be flagged as Hermitian. See typecasting matrices.\n\narg1 arg2 are real numbers.\n\nA special method is provided for real Diagonal matrices (2).\n\nSee also: invsqrt.\n\nExamples\n\nusing LinearAlgebra, PosDefManifold\nP=randP(5);     # use P=randP(ComplexF64, 5) for generating an Hermitian matrix\nQ=pow(P, 0.5);            # =>  QQ=P\nQ, W=pow(P, 0.5, -0.5);\nW*P*W ≈ I ? println(\" ⭐ \") : println(\" ⛔ \")\nQ*Q ≈ P ? println(\" ⭐ \") : println(\" ⛔ \")\nR, S=pow(P, 0.3, 0.7);\nR*S ≈ P ? println(\" ⭐ \") : println(\" ⛔ \")\n\n\n\n\n\n"
},
...

{
    ...
    "category": "function",
    "text": "(1) choL(P::ℍ)\n(2) choL(D::𝔻{T}) where T<:Real\n\n(1) Given a real or complex positive definite Hermitian matrix P,  return the Cholesky lower triangular factor L  such that LL^H=P. To obtain L^H or both L and L^H, use instead  julia function cholesky(P).\n\nOn output, L is of type LowerTriangular.\n\n(2) For a real Diagonal matrix D, return D^12.\n\nExamples\n\nusing PosDefManifold\nP=randP(5);\nL=choL(P);\nL*L\'≈ P ? println(\" ⭐ \") : println(\" ⛔ \")\n\n\n\n\n\n"
},
